Mi Currículum Vitae Web
Este proyecto es mi CV personal desarrollado de forma nativa para presentar mi perfil profesional, habilidades y proyectos como desarrolladora. El objetivo fue aplicar buenas prácticas de maquetación y diseño centrado en el usuario.

🚀 Demo en vivo
Puedes ver el resultado final aquí:
Ver mi CV en Vercel

🛠️ Tecnologías utilizadas
HTML5: Estructura semántica para mejorar la accesibilidad y el SEO.

CSS3: Estilos modernos, incluyendo Flexbox, CSS Grid y diseño Responsive (Mobile First).

JavaScript (ES6+): Funcionalidades interactivas básicas y manipulación del DOM.

📋 Características
Diseño adaptable a cualquier dispositivo (Móvil, Tablet, Desktop).

Secciones de perfil profesional, experiencia, formación y contacto.

Optimización de carga y rendimiento.
